...
// 1. START THE WEB SERVER FIRST (Prevents "Application failed to respond" crashes)
...
if (token) {
  try {
    ...
    bot.on("polling_error", (error) => {
      console.log("⚠️ Telegram Polling Error (Can be ignored if it reconnects):", error.message);
    });
...
app.post("/getlink", async (req, res) => {
  ...
  // Note: Your original script used GROUP_ID instead of channelId from the request, keeping it as GROUP_ID to match your logic
  try {
